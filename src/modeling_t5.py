...
class T5ClassificationHead_OCSR(nn.Module):

    def __init__(self, config: T5Config):
        super().__init__()
        self.dense_0 = nn.Linear(config.d_model + 1536, config.d_model // 4)
        self.norm_0 = T5LayerNorm(config.d_model // 4)
        self.relu_0 = NewGELUActivation()
        self.dropout_0 = nn.Dropout(p=config.classifier_dropout)
    ...
    def forward(self, hidden_states, swin_features) -> torch.Tensor:
        hidden_states = torch.cat([hidden_states, swin_features], dim=-1)
        hidden_states = self.dropout_0(
            self.relu_0(self.norm_0(self.dense_0(hidden_states)))
        )
        hidden_states = self.dropout_1(
...
class T5ClassificationHead_MolNexTR(nn.Module):
    def __init__(self, config: T5Config):
        super().__init__()
        self.dense_0 = nn.Linear(config.d_model + 1024, config.d_model // 4)
        self.norm_0 = T5LayerNorm(config.d_model // 4)
        self.relu_0 = NewGELUActivation()
        self.dropout_0 = nn.Dropout(p=config.classifier_dropout)
...
class T5PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, T5ClassificationHead_OCSR):
            module.dense_0.weight.data.normal_(
                mean=0.0, std=factor * ((self.config.d_model + 1536) ** -0.5)
            )
            if hasattr(module.dense_0, "bias") and module.dense_0.bias is not None:
                module.dense_0.bias.data.zero_()
        ...
        elif isinstance(module, T5ClassificationHead_MolNexTR):
            module.dense_0.weight.data.normal_(
                mean=0.0, std=factor * ((self.config.d_model + 1024) ** -0.5)
            )
            if hasattr(module.dense_0, "bias") and module.dense_0.bias is not None:
                module.dense_0.bias.data.zero_()
...
def contrastive_loss(embeddings, labels, temperature=0.1):
    ...
    Args:
        ...
        temperature: Scaling factor for logits in the contrastive loss.
    ...
    # Create a mask for positive samples (same class)
    labels = labels.unsqueeze(1)  # (batch_size, 1)
    positive_mask = labels.eq(labels.T).float()  # (batch_size, batch_size)

    # Exclude self-comparisons in the positive mask
    self_mask = torch.eye(batch_size, device=labels.device)
    positive_mask = positive_mask - self_mask

    # Compute log-softmax
    logits_masked = F.log_softmax(scaled_similarity, dim=1)

    # Compute loss
    positive_logits = logits_masked * positive_mask
    positive_sum = positive_logits.sum(dim=1)

    num_positives = positive_mask.sum(dim=1)
    loss_per_sample = -positive_sum / (num_positives + 1e-8)
...
class FocalLoss(nn.Module):
    def __init__(self, alpha=1.0, gamma=2.0, reduction="mean"):
        ...
        Args:
            alpha (float, optional): Weighting factor for the classes. Default: 1.0
    ...
    def forward(self, inputs, targets):
        ...
        targets_prob = (probs * targets_one_hot).sum(dim=1)

        # Compute the focal loss components
        focal_weight = self.alpha * (1 - targets_prob) ** self.gamma
        log_prob = torch.log(targets_prob + 1e-8)
...
class T5ForSequenceClassification(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config):
        ...
        if config.swin_used == "ocsr":
            self.classification_head = T5ClassificationHead_OCSR(config)
        elif config.swin_used == "molnextr":
            self.classification_head = T5ClassificationHead_MolNexTR(config)
        elif config.swin_used == "none":
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        swin_features: Optional[torch.FloatTensor] = None,
        ...
        if self.config.swin_used in ["ocsr", "molnextr"]:
            logits = self.classification_head(sentence_representation, swin_features)
        else:
        ...
        if labels is not None:
            ...
            if self.config.problem_type == "regression":
                ...
                if self.config.loss_function == "focal":
                    ...
                    raise ValueError(
                        f"Invalid loss function: {self.config.loss_function}."
                    )
                loss = loss_fct(
